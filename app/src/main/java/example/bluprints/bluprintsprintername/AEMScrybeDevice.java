...
public class AEMScrybeDevice 
{
	...
	public CardReader getCardReader(IAemCardScanner readerImpl) 
	{
		...
		return new CardReader(bluetoothSocket, readerImpl);
	}
	...
	public AEMPrinter getAemPrinter() 
	{
		...
		return new AEMPrinter(bluetoothSocket);
	}
	...
	public ArrayList<String> getPairedPrinters()
	{
		...
		for (BluetoothDevice device : pairedDevices) {
			if (device.getName() == null)
				continue;
			if ((device.getName().contains("rinter"))||(device.getName().contains("SPP"))||(device.getName().contains("MP57"))||(device.getName().contains("PP801"))||(device.getName().contains("NORM"))||(device.getName().contains("MPT"))||(device.getName().contains("Rfid"))||(device.getName().contains("SURAJ50"))||(device.getName().contains("TM-P20"))||(device.getName().contains("IB"))||(device.getName().contains("BluPrints")))
